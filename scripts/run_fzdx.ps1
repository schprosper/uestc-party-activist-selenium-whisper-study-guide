...
if ([string]::IsNullOrWhiteSpace($RunName)) { $RunName = "发展对象_自动播放转写" }
...
if (Test-Path -LiteralPath $PythonExe) {
    Write-Host "Starting fzdx.py with venv Python..."
    & $PythonExe -u .\fzdx.py
    exit $LASTEXITCODE
}

if (-not [string]::IsNullOrWhiteSpace($CondaExe) -and (Test-Path -LiteralPath $CondaExe)) {
    Write-Host "Starting fzdx.py with conda env $CondaEnv..."
    & $CondaExe run --no-capture-output -n $CondaEnv python -u .\fzdx.py
    exit $LASTEXITCODE
}
